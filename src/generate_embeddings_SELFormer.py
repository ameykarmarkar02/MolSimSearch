...
OUTPUT_CSV = os.path.join(DATA_DIR, "sample_embeddings_SELFormer.csv")
MODEL_NAME = "HUBioDataLab/SELFormer"  # switched to SELFormer model :contentReference[oaicite:0]{index=0}
BATCH_SIZE = 64
...
model = AutoModel.from_pretrained(
    MODEL_NAME,
    use_safetensors=True    # ← tell HF to pick the safetensors file
).to(device)
...
def main():
    ...
    with open(OUTPUT_CSV, "w", newline="") as fout:
        ...
        for start in range(0, n, BATCH_SIZE):
            ...
            # Write rows: [CID, emb_0, emb_1, ..., emb_N]
